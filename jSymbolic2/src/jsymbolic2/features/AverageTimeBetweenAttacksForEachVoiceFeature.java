...
public class AverageTimeBetweenAttacksForEachVoiceFeature
    ...
    public double[] extractFeature(Sequence sequence,
        ...
        if (null != sequence_info) {
            ...
            for (int chan = 0; 16 > chan; chan++) {
                // Mark with null if no notes contained in channel
                if (0 == sequence_info.channel_statistics[chan][0])
                    intervals[chan] = null;
                    ...
                    for (int tick = 0; tick < sequence_info.note_attack_tick_map.length; tick++) {
                        ...
                        boolean attack = sequence_info.note_attack_tick_map[tick][chan];

                        if (!attack)
                            time_so_far += sequence_info.duration_of_ticks_in_seconds[tick];
                        else {
                            if (-1 != tick_of_last_attack) {
            ...
            if (null == averages || 0 == averages.length)
                value = 0.0;
            else
