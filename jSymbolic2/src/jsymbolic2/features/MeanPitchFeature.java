    ...
    public double[] extractFeature(Sequence sequence,
        ...
        if (null != sequence_info) {
            ...
            for (int chan = 0; chan < sequence_info.channel_statistics.length; chan++) {
                if ((10 - 1) != chan) // Exclude the unpitched percussion Channel 10
                {
            ...
            if (0 == sequence_info.total_number_pitched_note_ons)
                value = 0.0;
            else
                value = cumulative_pitch_values / sequence_info.total_number_pitched_note_ons;
        } else value = -1.0;
