		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			int count = 0;
			for (int bin = 0; bin < sequence_info.getBeat_histogram_thresholded_table().length; bin++)
				if (sequence_info.getBeat_histogram_thresholded_table()[bin][2] > 0.001)
					count++;
			...
			{
				if (sequence_info.getBeat_histogram_thresholded_table()[peak_bins[bin]][2] > max_so_far)
				{
					max_so_far = sequence_info.getBeat_histogram_thresholded_table()[peak_bins[bin]][2];
			...
			for (int i = 0; i < peak_bins.length; i++)
			{
				...
				if (right_limit > sequence_info.getBeat_histogram_thresholded_table().length)
					right_limit = sequence_info.getBeat_histogram_thresholded_table().length;
