		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			int max_index = mckay.utilities.staticlibraries.MathAndStatsMethods.getIndexOfLargest(sequence_info.getMelodic_interval_histogram());

			// Find the second highest bin
			double second_max = 0;
			int second_max_index = 0;
			for (int bin = 0; bin < sequence_info.getMelodic_interval_histogram().length; bin++)
			{
				if ( sequence_info.getMelodic_interval_histogram()[bin] > second_max && bin != max_index )
				{
					second_max = sequence_info.getMelodic_interval_histogram()[bin];
					second_max_index = bin;
				}
			}

			// Calculate the value
			if (sequence_info.getMelodic_interval_histogram()[max_index] == 0.0)
				value = 0.0;
			else 
				value = sequence_info.getMelodic_interval_histogram()[second_max_index] /
						sequence_info.getMelodic_interval_histogram()[max_index];
		} 
		else value = -1.0;
