	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			for (int chan = 0; chan < sequence_info.getChannel_statistics().length; chan++)
			{
				...
				{
					if (sequence_info.getChannel_statistics()[chan][6] < min_so_far)
					{
						min_so_far = sequence_info.getChannel_statistics()[chan][6];
						lowest_chan = chan;
					}
				}
			}
			...
			double[] intervals = new double[sequence_info.getChannel_statistics().length - silent_count];
			int count = 0;
			for (int chan = 0; chan < sequence_info.getChannel_statistics().length; chan++)
			{
				if (sequence_info.getChannel_statistics()[chan][0] != 0 && chan != (10 - 1))
				{
					intervals[count] = (double) sequence_info.getChannel_statistics()[chan][3];
					count++;
				}
			}
			
			// Set value
			if (intervals == null || intervals.length == 0 || sequence_info.getChannel_statistics()[lowest_chan][3] == 0)
				value = 0.0;
			else
			{
				double average = mckay.utilities.staticlibraries.MathAndStatsMethods.getAverage(intervals);
				value = ((double) sequence_info.getChannel_statistics()[lowest_chan][3]) / ((double) average);
			}
		}
		else value = -1.0;
