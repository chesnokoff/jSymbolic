    ...
    public double[] extractFeature(Sequence sequence,
        ...
        if (null != sequence_info) {
            ...
            for (int chan = 0; chan < sequence_info.channel_statistics.length; chan++) {
                if (0 != sequence_info.channel_statistics[chan][6] && (10 - 1) != chan) {
            ...
            for (int chan = 0; chan < sequence_info.channel_statistics.length; chan++) {
                if (0 != sequence_info.channel_statistics[chan][0] && (10 - 1) != chan) {
                    intervals[count] = sequence_info.channel_statistics[chan][3];
                    count++;
                }
            }

            // Set value
            if (null == intervals || 0 == intervals.length || 0 == sequence_info.channel_statistics[lowest_chan][3])
                value = 0.0;
            else {
                double average = mckay.utilities.staticlibraries.MathAndStatsMethods.getAverage(intervals);
                value = sequence_info.channel_statistics[lowest_chan][3] / average;
            }
        } else value = -1.0;
