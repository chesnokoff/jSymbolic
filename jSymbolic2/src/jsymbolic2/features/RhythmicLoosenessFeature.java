		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			{
				// 30% of this peak
				double limit_value = 0.3 * sequence_info.getBeat_histogram()[peak_bins[peak]];
				...
				while (i >= 0)
				{
					if (sequence_info.getBeat_histogram()[i] < limit_value)
						i = -1;
					else
					{
						left_index = i;
				...
				i = peak_bins[peak];
				int right_index = 0;
				while (i < sequence_info.getBeat_histogram().length)
				{
					if (sequence_info.getBeat_histogram()[i] < limit_value)
						i = sequence_info.getBeat_histogram().length + 1;
					else
					{
						right_index = i;
						i++;
