    ...
    public double[] extractFeature(Sequence sequence,
        ...
        if (null != sequence_info) {
            ...
            for (int bin = 0; bin < sequence_info.beat_histogram_thresholded_table_120_bpm_standardized.length; bin++)
                if (0.001 < sequence_info.beat_histogram_thresholded_table_120_bpm_standardized[bin][2])
                    count++;
            ...
            for (int peak = 0; peak < peak_bins.length; peak++) {
                ...
                // Calculate width
                widths[peak] = (double) right_index - left_index;
            }

            // Calculate the feature value
            if (null == widths || 0 == widths.length)
                value = 0.0;
            else
