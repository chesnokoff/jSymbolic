		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			for (int bin = 0; bin < sequence_info.getBeat_histogram().length; bin++)
			{
				if (sequence_info.getBeat_histogram()[bin] > highest_magnitude)
				{
					highest_magnitude = sequence_info.getBeat_histogram()[bin];
			...
			for (int bin = 0; bin < sequence_info.getBeat_histogram_thresholded_table().length; bin++)
				if ( sequence_info.getBeat_histogram_thresholded_table()[bin][1] > second_highest_magnitude &&
				     bin != index_of_bin_with_highest_magnitude )
				{
					second_highest_magnitude = sequence_info.getBeat_histogram_thresholded_table()[bin][1];
				}
