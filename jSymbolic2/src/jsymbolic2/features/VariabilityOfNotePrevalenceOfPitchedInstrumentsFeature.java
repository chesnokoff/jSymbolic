		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			for (int instrument = 0; instrument < sequence_info.getPitched_instrument_prevalence().length; instrument++)
			{
				if (sequence_info.getPitched_instrument_prevalence()[instrument][0] != 0)
				{
					instruments_present++;
				}
			}
			...
			for (int instrument = 0; instrument < sequence_info.getPitched_instrument_prevalence().length; instrument++)
			{
				if (sequence_info.getPitched_instrument_prevalence()[instrument][0] != 0)
				{
					instrument_frequencies[count] = (double) sequence_info.getPitched_instrument_prevalence()[instrument][0];
					count++;
				}
			}
