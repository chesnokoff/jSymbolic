...
public class VariabilityOfNumberOfIndependentVoicesFeature
    ...
    public double[] extractFeature(Sequence sequence,
        ...
        if (null != sequence_info) {
            ...
            int count = 0;
            for (int i = 0; i < number_sounding.length; i++) {
                if (0.5 < number_sounding[i]) {
                    final_number_sounding[count] = number_sounding[i];
                    count++;
                }
            }

            // Calculate the standard deviation
            if (null == final_number_sounding || 0 == final_number_sounding.length)
                value = 0.0;
            else
