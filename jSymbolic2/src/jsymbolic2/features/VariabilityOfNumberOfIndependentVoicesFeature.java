		...
		extends MIDIFeatureExtractor
{
	...
	throws Exception
	{
		...
		if (sequence_info != null)
		{
			...
			int[] number_sounding = new int[sequence_info.getNote_sounding_on_a_channel_tick_map().length];
			for (int i = 0; i < number_sounding.length; i++)
				number_sounding[i] = 0;

			// Find the number of voices sounding at each tick
			int rest_count = 0;
			for (int tick = 0; tick < sequence_info.getNote_sounding_on_a_channel_tick_map().length; tick++)
			{
				for (int chan = 0; chan < sequence_info.getNote_sounding_on_a_channel_tick_map()[tick].length; chan++)
				{
					if (sequence_info.getNote_sounding_on_a_channel_tick_map()[tick][chan])
