    ...
    public double[] extractFeature(Sequence sequence,
        ...
        if (null != sequence_info) {
            ...
            for (int tick = 0; tick < pitches_present_by_tick_excluding_rests.length; tick++) {
                ...
                // third
                if (1 < pitches_this_tick.length) {
                    for (int i = 0; i < pitches_this_tick.length; i++) {
                        for (int j = 0; j < i; j++) {
                            if (3 == (pitches_this_tick[i] - pitches_this_tick[j]) % 12) {
            ...
            if (0 == pitches_present_by_tick_excluding_rests.length)
                value = 0.0;
            else
                value = ticks_with_minor_third / pitches_present_by_tick_excluding_rests.length;
        } else value = -1.0;
