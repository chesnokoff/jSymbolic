 ...
 */
public enum FeatureExtractorAccess {
    ;
    /* FIELDS ***********************************************************************************************/
    ...
    static {
        ...
        MIDIFeatureExtractor[] all_extractors = FeatureExtractorAccess.all_implemented_feature_extractors;
        for (MIDIFeatureExtractor fe : all_extractors)
            FeatureExtractorAccess.names_of_all_implemented_features.add(fe.getFeatureDefinition().name);

        names_of_default_features_to_save = new ArrayList<>();
        for (int i = 0; i < FeatureExtractorAccess.default_features_to_save.length; i++)
            if (default_features_to_save[i])
                FeatureExtractorAccess.names_of_default_features_to_save.add(FeatureExtractorAccess.names_of_all_implemented_features.get(i));

        names_of_mei_specific_features = new ArrayList<>();
        for (MIDIFeatureExtractor feature : FeatureExtractorAccess.all_implemented_feature_extractors) {
            if (feature instanceof MEIFeatureExtractor) {
                String mei_feature_name = feature.getFeatureDefinition().name;
                FeatureExtractorAccess.names_of_mei_specific_features.add(mei_feature_name);
            }
        }

        // Validation and testing
        FeatureExtractorAccess.printWarningReportIfFeaturesAddedImproperly();
        //printAllFeatures();
    ...
    public static MIDIFeatureExtractor[] getAllImplementedFeatureExtractors() {
        return FeatureExtractorAccess.all_implemented_feature_extractors;
    }
    ...
    public static boolean[] getDefaultFeaturesToSave() {
        return FeatureExtractorAccess.default_features_to_save;
    }
    ...
    public static List<String> getNamesOfAllImplementedFeatures() {
        return FeatureExtractorAccess.names_of_all_implemented_features;
    }
    ...
    public static List<String> getNamesOfDefaultFeaturesToSave() {
        return FeatureExtractorAccess.names_of_default_features_to_save;
    }
    ...
    public static List<String> getNamesOfMeiSpecificFeatures() {
        return FeatureExtractorAccess.names_of_mei_specific_features;
    }
    ...
    public static String getFeatureCatalogueOverviewReport(boolean[] features_to_include) {
        ...
        if (null == features_to_include) {
            features_to_include = new boolean[FeatureExtractorAccess.all_implemented_feature_extractors.length];
        ...
        for (int i = 0; i < FeatureExtractorAccess.all_implemented_feature_extractors.length; i++) {
            if (features_to_include[i]) {
                MIDIFeatureExtractor feat = FeatureExtractorAccess.all_implemented_feature_extractors[i];

                total_unique_features++;

                total_feature_dimensions += feat.getFeatureDefinition().dimensions;
                if (1 == feat.getFeatureDefinition().dimensions)
    ...
    private static void printWarningReportIfFeaturesAddedImproperly() {
        ...
        String[] names_of_all_features_added = FeatureExtractorAccess.names_of_all_implemented_features.toArray(new String[FeatureExtractorAccess.names_of_all_implemented_features.size()]);

        // Verify that no features have been added to all_implemented_feature_extractors that are dependent on
        // features that have not themselves been added to all_implemented_feature_extractors
        for (int feat = 0; feat < FeatureExtractorAccess.all_implemented_feature_extractors.length; feat++) {
            String[] dependencies = FeatureExtractorAccess.all_implemented_feature_extractors[feat].getDepenedencies();
        ...
        // all_implemented_feature_extractors
        String[] all_feature_codes = new String[FeatureExtractorAccess.all_implemented_feature_extractors.length];
        for (int i = 0; i < all_feature_codes.length; i++)
            all_feature_codes[i] = FeatureExtractorAccess.all_implemented_feature_extractors[i].getFeatureCode();
        int[][] duplicate_codes = mckay.utilities.staticlibraries.StringMethods.getIndexesOfDuplicateEntries(all_feature_codes);
        if (null != duplicate_codes) {
            for (int i = 0; i < duplicate_codes.length; i++) {
                String duplicated_feature_code = FeatureExtractorAccess.all_implemented_feature_extractors[duplicate_codes[i][0]].getFeatureCode();
        ...
        for (int feat = 0; feat < FeatureExtractorAccess.all_implemented_feature_extractors.length; feat++) {
            try {
                String[] split_code = FeatureExtractorAccess.all_implemented_feature_extractors[feat].getFeatureCode().split("-");
                ...
                if (0 != feat) {
                    if (this_group.equals(last_group)) {
                        if (this_number != (last_number + 1))
                            problem_report += "WARNING: The feature " + FeatureExtractorAccess.all_implemented_feature_extractors[feat].getFeatureCode() + " has been added to jSymbolic out of sequence (its code does not numerically follow the previous feature in its group). This is not a serious problem, but it could result in confusion.\n";
                    } else if (1 != this_number)
                        problem_report += "WARNING: The feature " + FeatureExtractorAccess.all_implemented_feature_extractors[feat].getFeatureCode() + " has been added to jSymbolic out of sequence (a new feature group should be numbered as 0). This is not a serious problem, but it could result in confusion.\n";
                }

                last_group = this_group;
                last_number = this_number;
            } catch (Exception e) {
                problem_report += "WARNING: The feature " + FeatureExtractorAccess.all_implemented_feature_extractors[feat].getFeatureCode() + " has an improperly formatted code. The code should consist of one or more letters identifying the feature group the feature belongs to, followed by a hyphen, followed by the number of the feature within that group. For example, a code of I-7 would be appropriate for the seventh feature of the Instrumentation feature group. This is not a serious problem, but it could result in confusion.\n";
            }
        }
        ...
        // to be extracted.
        if (FeatureExtractorAccess.default_features_to_save.length != FeatureExtractorAccess.all_implemented_feature_extractors.length)
            problem_report += "WARNING: " + FeatureExtractorAccess.all_implemented_feature_extractors.length + " features have been added to jSymbolic, but settings for " + FeatureExtractorAccess.default_features_to_save.length + " features have been specified as to which features are to be extracted and saved by default. These numbers should be identical. This is a serious problem, as a mismatch could cause jSymbolic to crash.\n";
        else for (int feat = 0; feat < FeatureExtractorAccess.all_implemented_feature_extractors.length; feat++) {
            if (!FeatureExtractorAccess.default_features_to_save[feat]) {
                if (1 == all_implemented_feature_extractors[feat].getFeatureDefinition().dimensions &&
                        !(FeatureExtractorAccess.all_implemented_feature_extractors[feat] instanceof MEIFeatureExtractor))
                ...
                if (FeatureExtractorAccess.all_implemented_feature_extractors[feat] instanceof MEIFeatureExtractor)
    ...
    private static void printAllFeatures() {
        System.out.println("ALL " + FeatureExtractorAccess.all_implemented_feature_extractors.length + " IMPLEMENTED FEATURES:");
        for (int i = 0; i < FeatureExtractorAccess.all_implemented_feature_extractors.length; i++)
            System.out.println((i + 1) + ":\t" + FeatureExtractorAccess.all_implemented_feature_extractors[i].getFeatureCode() + "\t" +
                    FeatureExtractorAccess.all_implemented_feature_extractors[i].definition.name);
    }
    ...
    private static void printAllFeatureDependencies() {
        ...
        for (int i = 0; i < FeatureExtractorAccess.all_implemented_feature_extractors.length; i++) {
            if (null != all_implemented_feature_extractors[i].dependencies) {
                System.out.println(FeatureExtractorAccess.all_implemented_feature_extractors[i].getFeatureCode() + "\t" +
                        FeatureExtractorAccess.all_implemented_feature_extractors[i].definition.name);
                for (int j = 0; j < FeatureExtractorAccess.all_implemented_feature_extractors[i].dependencies.length; j++)
                    System.out.println("\tDEPENDS ON: " + FeatureExtractorAccess.all_implemented_feature_extractors[i].dependencies[j]);
            }
        }
    }
}