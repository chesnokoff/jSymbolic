...
public class MIDIReporter {
    ...
    public MIDIReporter(File file_to_parse)
        ...
        try {
            ...
            if (SymbolicMusicFileUtilities.isValidMeiFile(file_to_parse))
                ...
                throw new Exception("The specified file, " + file_to_parse + ", is not a valid readable MIDI or MEI file.");
        } catch (IOException e) {
        ...
        for (int tr = 0; tr < tracks.length; tr++)
            if (0 != number_active_channels_per_track[tr])
    ...
    public static String prepareConsistencyReports(File[] files_to_check,
        ...
        for (int file = 0; file < files_to_check.length; file++) {
                ...
                for (int ev = 0; ev < tracks[tr].size(); ev++) {
                    ...
                    if (message instanceof MetaMessage) {
                        ...
                        if (0x51 == ((MetaMessage) message).getType()) {
                            ...
                            int tempo_bpm = Math.round(60000000.0f / ms_tempo_float);

                            // Note the tempo
                            tempo_values.add(Integer.toString(tempo_bpm));
                        }
                        ...
                        if (0x58 == ((MetaMessage) message).getType()) {
                            ...
                            int time_signature_numerator = data[0] & 0xFF;
                            int time_signature_denominator = 1 << (data[1] & 0xFF);
                        ...
                        if (0x59 == ((MetaMessage) message).getType()) {
                            ...
                            if (0 == data[1]) key_signture_info = "Major with ";
                            ...
                            if (0 == number_accidentals)
                                key_signture_info += (0 + " accidentals");
                            else if (0 > number_accidentals)
                    ...
                    if (message instanceof ShortMessage short_message) {
                        ...
                        if (0xc0 == short_message.getCommand()) {
                            ...
                            general_midi_patch_values.add(Integer.toString(patch_number));

                            // Note the patch in this particular track and channel
                            patches_per_channel_per_track[tr][channel].add(Integer.toString(patch_number));
                        }

                        // If this is a MIDI Channel Volume message
                        if (0xb0 == short_message.getCommand() && 7 == short_message.getData1()) {
                            // The channel volume
                            long channel_volume = short_message.getData2();

                            // Note the channel volume
                            channel_volume_values.add(Long.toString(channel_volume));
                        }
                        ...
                        if (0x90 == short_message.getCommand()) // note on
                        {
                            if (0 != short_message.getData2()) // not velocity 0
                            {
                                ...
                                // Note the General MIDI patch
                                velocity_values.add(Integer.toString(velocity));
            ...
            for (int track = 0; track < patches_per_channel_per_track.length; track++)
                for (int chan = 0; chan < patches_per_channel_per_track[track].length; chan++) {
                    if (0 != note_on_count[track][chan]) {
                        ...
                        if (null != patches_this_channel_per_track_doubles_removed &&
                                0 != patches_this_channel_per_track_doubles_removed.length) {
                            multiplexed_voices_on_this_track_channel = patches_this_channel_per_track_doubles_removed.length;
                            if (1 < multiplexed_voices_on_this_track_channel)
                                multiplexed_track_channels.add("Track " + track + " Chan " + chan);
                        }
                        total_separate_voices_count += multiplexed_voices_on_this_track_channel;
                    }
                }
            unique_voices_found[file][0] = Long.toString(total_separate_voices_count);
            ...
            double[] rhythmic_value_offsets = MIDIReporter.getRhytmicValueOffsets(reporters[file].sequence);
            ...
            for (int ch = 0; ch < tracks_using_each_channel.length; ch++)
                if (1 < tracks_using_each_channel[ch].size())
                    chans_duplicated_on_multiple_tracks.add(Integer.toString(ch));
        ...
        if (check_files_individually) {
            ...
            for (int file = 0; file < files_to_check.length; file++) {
                ...
                report.append("\tNUMBER OF VOICES: " + MIDIReporter.getIntraConsistencyReport(unique_voices_found[file], verbose, "Number Of Voices", inconsistencies_found[file]) + "\n");
                report.append("\tTEMPO: " + MIDIReporter.getIntraConsistencyReport(unique_tempos_found[file], verbose, "Tempo", inconsistencies_found[file]) + "\n");
                report.append("\tTIME SIGNATURE: " + MIDIReporter.getIntraConsistencyReport(unique_time_signatures_found[file], verbose, "Time Signature", inconsistencies_found[file]) + "\n");
                report.append("\tKEY SIGNATURE: " + MIDIReporter.getIntraConsistencyReport(unique_key_signatures_found[file], verbose, "Key Signature", inconsistencies_found[file]) + "\n");
                report.append("\tGENERAL MIDI PATCHES: " + MIDIReporter.getIntraConsistencyReport(unique_general_midi_patches_found[file], verbose, "General MIDI Patches", inconsistencies_found[file]) + "\n");
                report.append("\tUNPITCHED CHANNEL 10 NOTES: " + MIDIReporter.getIntraConsistencyReport(is_channel_10_used[file], verbose, "Unpitched Channel 10 Notes", inconsistencies_found[file]) + "\n");
                report.append("\tNOTE VELOCITIES: " + MIDIReporter.getIntraConsistencyReport(unique_velocities_found[file], verbose, "Note Velocities", inconsistencies_found[file]) + "\n");
                report.append("\tCHANNEL VOLUMES: " + MIDIReporter.getIntraConsistencyReport(unique_channel_volumes_found[file], verbose, "Channel Volumes", inconsistencies_found[file]) + "\n");
                report.append("\tPITCH BEND VALUES: " + MIDIReporter.getIntraConsistencyReport(unique_pitch_bend_values_found[file], verbose, "Pitch Bend Values", inconsistencies_found[file]) + "\n");
                report.append("\tMEDIAN RHYTHMIC OFFSET: " + MIDIReporter.getIntraConsistencyReport(unique_median_rhythmic_offsets_found[file], verbose, "Median Rhythmic Offset", inconsistencies_found[file]) + "\n");
                report.append("\tRHYTHMIC OFFSET STDEV: " + MIDIReporter.getIntraConsistencyReport(unique_rhythmic_offsets_stdev_found[file], verbose, "Rhythmic Offset STDEV", inconsistencies_found[file]) + "\n");
                report.append("\tDUPLICATED MIDI CHANNELS: " + MIDIReporter.getIntraConsistencyReport(channels_duplicated_on_multiple_tracks[file], verbose, "Duplicated MIDI Channels", inconsistencies_found[file]) + "\n");
                report.append("\tMULTIPLEXED MIDI TRACKS AND CHANNELS: " + MIDIReporter.getIntraConsistencyReport(unique_multiplexed_track_channels_found[file], verbose, "Multiplexed MIDI Tracks and Channels", inconsistencies_found[file]) + "\n");
                report.append("\tMIDI FILE TYPE: " + MIDIReporter.getIntraConsistencyReport(unique_mid_file_types_found[file], verbose, "MIDI File Type", inconsistencies_found[file]) + "\n");
            }
            report.append("\n");

            // Prepare summary intraconsistency report
            if (1 < files_to_check.length) {
        ...
        if (check_across_files && 1 < files_to_check.length) {
            report.append("\n************ SUMMARY FILE INTERCONSISTENCY REPORT ************\n\n");
            report.append("\tNUMBER OF VOICES: " + MIDIReporter.getInterConsistencyReport(unique_voices_found, files_to_check, verbose) + "\n");
            report.append("\tTEMPO: " + MIDIReporter.getInterConsistencyReport(unique_tempos_found, files_to_check, verbose) + "\n");
            report.append("\tTIME SIGNATURE: " + MIDIReporter.getInterConsistencyReport(unique_time_signatures_found, files_to_check, verbose) + "\n");
            report.append("\tKEY SIGNATURE: " + MIDIReporter.getInterConsistencyReport(unique_key_signatures_found, files_to_check, verbose) + "\n");
            report.append("\tGENERAL MIDI PATCHES: " + MIDIReporter.getInterConsistencyReport(unique_general_midi_patches_found, files_to_check, verbose) + "\n");
            report.append("\tUNPITCHED CHANNEL 10 NOTES: " + MIDIReporter.getInterConsistencyReport(is_channel_10_used, files_to_check, verbose) + "\n");
            report.append("\tNOTE VELOCITIES: " + MIDIReporter.getInterConsistencyReport(unique_velocities_found, files_to_check, verbose) + "\n");
            report.append("\tCHANNEL VOLUMES: " + MIDIReporter.getInterConsistencyReport(unique_channel_volumes_found, files_to_check, verbose) + "\n");
            report.append("\tPITCH BEND VALUES: " + MIDIReporter.getInterConsistencyReport(unique_pitch_bend_values_found, files_to_check, verbose) + "\n");
            report.append("\tMEDIAN RHYTHMIC OFFSET: " + MIDIReporter.getInterConsistencyReport(unique_median_rhythmic_offsets_found, files_to_check, verbose) + "\n");
            report.append("\tRHYTHMIC OFFSET STDEV: " + MIDIReporter.getInterConsistencyReport(unique_rhythmic_offsets_stdev_found, files_to_check, verbose) + "\n");
            report.append("\tDUPLICATED MIDI CHANNELS: " + MIDIReporter.getInterConsistencyReport(channels_duplicated_on_multiple_tracks, files_to_check, verbose) + "\n");
            report.append("\tMULTIPLEXED MIDI TRACKS AND CHANNELS: " + MIDIReporter.getInterConsistencyReport(unique_multiplexed_track_channels_found, files_to_check, verbose) + "\n");
            report.append("\tMIDI FILE TYPE: " + MIDIReporter.getInterConsistencyReport(unique_mid_file_types_found, files_to_check, verbose) + "\n");
            report.append("\n");
        }
    ...
    public String prepareHeaderReport()
        ...
        if (null != midi_file_format &&
                null != midi_file_format.getProperty("title") &&
                null != midi_file_format.getProperty("author") &&
                null != midi_file_format.getProperty("copyright") &&
                null != midi_file_format.getProperty("date") &&
                null != midi_file_format.getProperty("comment")) {
            report.append("\nFILE METADATA:\n");
        ...
        long total_number_beats = Math.round((double) sequence.getTickLength() / ppqn_ticks_per_beat);
        ...
        for (int tr = 0; tr < tracks.length; tr++) {
            ...
            for (int ev = 0; ev < tracks[tr].size(); ev++) {
                ...
                if (message instanceof MetaMessage) {
                    ...
                    if (0x03 == ((MetaMessage) message).getType() || // Track name
                            0x04 == ((MetaMessage) message).getType()) // Instrument name
                    {
                        // Interpret the field
                        if (0x03 == ((MetaMessage) message).getType())
                            temp_track_and_instrument_names_report.append("\tTRACK");
                        else if (0x03 == ((MetaMessage) message).getType())
                        ...
                        temp_track_and_instrument_names_report.append(" NAME FOR TRACK " + (tr + 1) + ": \"" +
                                this_report + "\", Beat: " + getBeatNumber(event_tick) + ", Tick " +
                                (event_tick + 1) + ", Track " + (tr + 1) + "\n");
                    ...
                    if (0x01 == ((MetaMessage) message).getType()) {
                        // Interpret the data
                        StringBuilder this_report = new StringBuilder();
                        for (int ch = 0; ch < data.length; ch++) {
                    ...
                    if (0x05 == ((MetaMessage) message).getType()) {
                        // Interpret the data
                        StringBuilder this_report = new StringBuilder();
                        for (int ch = 0; ch < data.length; ch++) {
                            char this_character = (char) data[ch];
            ...
            if (found_tempo)
                tempo_report.append(temp_tempo_report);
            if (found_time_signature)
                time_signature_report.append(temp_time_signature_report);
            if (found_key_signature)
                key_signature_report.append(temp_key_signature_report);
            if (found_track_or_instrument_name)
                track_and_instrument_names_report.append(temp_track_and_instrument_names_report);
            if (found_text)
                text_report.append(temp_text_report);
            if (found_lyrics)
                lyrics_report.append(temp_lyrics_report);
        }
    ...
    public String prepareNoteReport(boolean temporal_order, boolean include_pitch_bends)
                                ...
                                if (channel == ch) {
                                    ...
                                    if (0x90 == short_message.getCommand()) // note on
                                    {
                                        if (0 != short_message.getData2()) // not velocity 0
                                        {
                                            ...
                                            // Note On
                                            long end_tick = MIDIReporter.getTickOfNoteOff(track, ev, channel, pitch);
                                    ...
                                    else if (include_pitch_bends && 0xe0 == short_message.getCommand()) {
                                        // The (second byte) value of the pitch bend (the less significant
                                        // first byte is ignored)
                                        int pitch_bend_value = short_message.getData2();
        ...
        // Report MIDI Note Ons in the order they occur (not segregated by MIDI track or MIDI channel)
            ...
            List<NoteInfo> temporally_sorted_notes = CollectedNoteInfo.noteListToSortedNoteList(collected_note_info.getNoteList());
    ...
    private String getBeatNumber(long tick) {
        DecimalFormat formatter = new java.text.DecimalFormat("#.###");
        double beat = (double) tick / ppqn_ticks_per_beat;
        beat += 1.0;
                ...
                if (message instanceof ShortMessage short_message) {
                    ...
                    if (0x90 == short_message.getCommand()) {
                        if (0 != short_message.getData2()) // not velocity 0
                        {
                            ...
                            int event_end_tick = MIDIReporter.getTickOfNoteOff(short_message, ev, track);
    ...
    private static String getIntraConsistencyReport(String[] to_check,
        ...
        if (report_values) {
            String values = "";
            if (null == to_check || (1 == to_check.length && null == to_check[0]))
            ...
            if (null != inconsistencies_found_note)
    ...
    private static String getInterConsistencyReport(String[][] unique_values_per_file,
        ...
        for (int file = 0; file < unique_values_per_file.length; file++) {
            ...
            if (null == unique_values_per_file[file] ||
                    (1 == unique_values_per_file[file].length && null == unique_values_per_file[file][0])) {
        ...
        if (1 == unique_value_sets_found_so_far_all_files.size())
