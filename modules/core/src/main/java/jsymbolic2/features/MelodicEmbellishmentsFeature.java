 ...
 */
public class MelodicEmbellishmentsFeature extends Feature {
                ...
                channel != 10 - 1) {
                    ...
                    for (NoteInfo current_note : all_notes_in_this_channel) {
                        ...
                        if (current_tick_index > 0 && current_tick_index < sorted_note_on_ticks_this_channel.length - 1) {
                            ...
                            for (NoteInfo previous : previous_notes) if (previous.getDuration() >= 3 * current_note.getDuration())
                                previous_check = true;
                            boolean next_check = false;
                            for (NoteInfo next : next_notes) if (next.getDuration() >= 3 * current_note.getDuration())
                                next_check = true;
