    ...
    public double[] extractFeature(Sequence sequence, MIDIIntermediateRepresentations sequence_info, double[][] other_feature_values) throws Exception {
        ...
        if (sequence_info != null) {
            ...
            for (int channel = 0; channel < 16; channel++) {
                if (// Skip over the unpitched percussion channel
                channel != (10 - 1)) {
                    ...
                    for (NoteInfo current_note : all_notes_in_this_channel) {
                        ...
                        for (Integer tick_following_note : sorted_note_on_ticks_this_channel) {
                            if (!found_repeated_pitch && tick_following_note > current_note.getStartTick()) {
                                ...
                                for (NoteInfo following_note : notes_starting_this_following_tick) {
                                    if (current_note.getPitch() == following_note.getPitch() && !found_repeated_pitch && notes_gone_by_with_different_pitch <= max_notes_that_can_go_by) {
                            ...
                            if (found_repeated_pitch || notes_gone_by_with_different_pitch > max_notes_that_can_go_by)
                                break;
                        }
                    }
                }
