...
class ConfigRun implements Consumer<String[]> {
    ...
    public void accept(String[] args) {
        printStreams = new PrintStreams(printStreams.statusPrintStream(), printStreams.errorPrintStream());
    ...
    private static void processFiveArguments(String[] args, String config_file_path, PrintStreams printStreams) {
        ...
        try {
            ...
            ConfigurationFileData config_file_data = new ConfigurationFileValidatorTxtImpl().parseConfigFile(config_file_path, config_file_headers_to_check, printStreams.errorPrintStream());
    ...
    private void processWithConfigFileOnly(String config_file_path, PrintStreams printStreams) throws Exception {
        UserFeedbackGenerator.printParsingConfigFileMessage(printStreams.statusPrintStream(), config_file_path);
        ConfigurationFileData config_file_data = new ConfigurationFileValidatorTxtImpl().parseConfigFileAllHeaders(config_file_path, printStreams.errorPrintStream());
